...
export function InvoiceForm({ clinicId, patients }: InvoiceFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (items.some((item) => !item.description || item.unitPrice <= 0)) {
      newErrors.items = "Fill in all line items with valid amounts";
    }
          ...
          <div className="space-y-3">
            ...
            {items.map((item, index) => (
              <div key={index} className="grid grid-cols-[1fr_80px_100px_80px_32px] gap-2 items-center">
                ...
                />
                <span className="text-sm font-medium text-[var(--color-text-primary)] text-right pr-2">
